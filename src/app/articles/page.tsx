      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <div className={`bg-white rounded-xl shadow-sm border overflow-hidden transition-all duration-300 ${postedLinkedIn ? 'border-green-200' : 'border-gray-100'}`}>
          ...
          <div className="px-6 py-4 border-t border-gray-100"
            style={{ background: postedLinkedIn ? '#f0fdf4' : '#fafafa' }}>
            <label className="flex items-center gap-3 cursor-pointer select-none group" onClick={toggleLinkedIn}>
              <div
                ...
                style={postedLinkedIn
                  ? { background: '#00BFA6' }
                  : { border: '2px solid #D4A830', background: '#fff' }}
              >
                {postedLinkedIn && (
              ...
              <span className="text-sm font-bold" style={{ color: postedLinkedIn ? '#15803d' : '#0A1929' }}>
                {postedLinkedIn ? "Posted to LinkedIn ✓" : "Mark as posted to LinkedIn"}
              </span>
            </label>
          </div>
        </div>
